...
- `gateway/app/main.py:423` — `ui_home` (`GET /`) — groups records by local date, paginates by date count (`ui_show_count`).
...
- `gateway/app/main.py:550` — `POST /records` (`ui_create`) — add via form; the form's `activity` `<select>` lets any configured activity be logged manually. Instant activities are stored closed (`stop_epoch = start_epoch`); timed ones keep the submitted stop (with the next-day wrap when stop < start) only when the duration is 30 minutes or less.
