...
- `POST /api/records` — create a record from JSON (`RecordIn`); `start` is required for wire compatibility. For feeding it means the end time (an explicit `stop` wins) and the stored epochs are made equal. Other timed activities retain start/stop normalization and the 30-minute guard.
- `PATCH /api/records/{rid}` — partial update. Feeding time edits keep both epochs equal; changing a session to feeding collapses it at its end. Other timed records retain the 30-minute guard.
