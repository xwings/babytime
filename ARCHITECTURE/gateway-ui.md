...
- `gateway/app/static/style.css` — `@media (max-width: 640px)` mobile layout (the only breakpoint; desktop is the default). The records `<table>` can't reflow into columns on a phone, so each `<tr>` becomes a labeled card: `thead` is hidden, `table/tbody/tr/td` go `display:block`, and each `td` shows its column name via `td::before { content: attr(data-label) }` — the labels come from `data-label="{{ t('col_*') }}"` on the cells in `index.html` (first cell, the checkbox, is unlabeled). Inputs bump to `font-size:16px` to stop iOS focus-zoom; header/tabs wrap full-width, `section`/`main` padding shrinks, `.bulk-actions` buttons stretch, and `.row-form` fields go full-width.
...
- Each row has an activity dropdown and a free-text Note field that
  ...
  entered stop. Rows for an instant activity show `—` for Stop and
